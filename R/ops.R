...
## NOTE:
## XeTeX uses x_glyph or x_string instead of set_char
## XeTeX uses x_fnt_def instead of fnt_def
...
op_x_font_def <- function(op, state) {
    ...
    if (is.null(fonts[[fontnum]]) ||
        ...
        fontsize <- 72.27*fromTeX(blockValue(op$block$op.opparams.ptsize),
                                  pre)/25.4
...
setGlyphs <- function(op, state) {
    ...
    for (i in 1:nGlyphs) {
        id <- glyphIds[i]
        bbox <- TeXglyphBounds(id, font$file, font$size, fontLib, pre)
        width <- TeXglyphWidth(id, font$file, font$size, fontLib, pre)
